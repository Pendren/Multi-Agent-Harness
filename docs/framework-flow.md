...
- **Prerequisites:** intent.md, context.md, and at least one (ideally all) numbered task spec(s). **orchestration.md**, **`.agent/skills/`** (development, test-author, test-runner), and **Cursor** adapter folders **`.cursor/skills/...`** must exist (from template or init).
